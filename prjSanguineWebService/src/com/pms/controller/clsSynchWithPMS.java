...
public class clsSynchWithPMS {
					 ...
					 if (systemDate.compareTo(WebStockExpiryDate)<=0) 
					 {
						 if(userCode.equalsIgnoreCase("SANGUINE"))
						 {
						        ...
						        else
						        {
						        	try
						        	{
						        		sqlMMS="SELECT a.strUserCode,a.strUserName,a.strType FROM tbluserhd a "
						        				+ "WHERE a.strUserCode='"+userCode+"' AND a.strClientCode='"+clientCode+"'; ";
						        		rsMMS = stmms.executeQuery(sql); 
										if(rsMMS.next())
										{
											jObj.put("UserStatus", true);
										}
	...
	public void funGetDashHeaderData(String POSCode, String PMSDate, String clientCode, Statement st, JSONArray arrObj, NumberFormat decformat) throws Exception
	{
        ...
        {
        	  //intTotalNoArrival=rsHeaderInfo1.getInt(1)+intTotalWalkNo;
              obj.put("TotalNoArrival",rsHeaderInfo1.getInt(1));
        }
        rsHeaderInfo1.close();
        //obj.put("Inhouse",intTotalNoArrival);
       // obj.put("Arrival", obj.get("TotalNumberOfArrival")+"/"+obj.get("TotalNumberOfReservation"));
	...
	public void fungetDashCountsData(String POSCode, String PMSDate, String clientCode, Statement st, JSONArray arrObj, NumberFormat decformat,  JSONObject obj) throws Exception
	{
       ...
       sql="SELECT count(*) FROM tblroomcancelation a,tblreservationhd b,tblguestmaster c,tblreservationdtl d,tblbookingtype e,"
	...
	public void funGetDashTotalRevenueData(String POSCode, String PMSDate, String clientCode, Statement st, JSONArray arrObj, NumberFormat decformat) throws Exception
	{
	 ...
	 String sql="select Count(*),SUM(a.dblGrandTotal) FROM tblbillhd a where date(a.dteBillDate)='"+PMSDate+"';";
	 JSONObject obj=new JSONObject();
	 ResultSet rsHeaderInfo = st.executeQuery(sql);
	...
	@GET
	@Path("/funGetRoomTypeCount")
	@Consumes(MediaType.APPLICATION_JSON)
	public JSONObject funGetRoomTypeCount(@QueryParam("POSCode") String POSCode,@QueryParam("clientCode") String clientCode,@QueryParam("PMSDate") String PMSDate)
	{
		JSONObject jObjStayViewData=new JSONObject();
		clsDatabaseConnection objDb=new clsDatabaseConnection();
	...
	@GET
	@Path("/funGetRoomStatusData")
	public JSONObject funGetRoomStatusData(@QueryParam("strRoomTypeDesc") String strRoomTypeDesc,@QueryParam("clientCode") String clientCode,@QueryParam("PMSDate") String PMSDate)
	{
		JSONObject jObjStayViewData=new JSONObject();
		clsDatabaseConnection objDb=new clsDatabaseConnection();
		JSONArray mainArrObj=new JSONArray();
		...
		try{
            ...
            st = pmsCon.createStatement();
			JSONObject obj=new JSONObject() ;
		    String sql=" Select s.roomdesc,s.fname,s.lname,s.roomtype,s.roomstatus,s.checkindate,s.deptdate,s.arrivaltme,"
		    		+ " s.depttime,s.staySingleday from"
		    		+ " (select a.strRoomDesc as roomdesc,d.strFirstName as fname,d.strLastName as lname,"
		    		+ " a.strRoomTypeDesc as roomtype ,a.strStatus as roomstatus ,date(b.dteCheckInDate) as checkindate, date(b.dteDepartureDate) as deptdate"
		    		...
		    		+ " and a.strRoomTypeDesc='"+strRoomTypeDesc+"' "
		    		+ " UNION"
		    		+ " Select p.strRoomDesc as roomdesc,'' as fname,'' as lname,'' as roomtype,'Free' as roomstatus, '' as checkindate,'' as deptdate ,'' as arrivaltme,'' as depttime,'' as staySingleday  "
		    		+ " from tblroom p  WHERE p.strStatus in('Free','Occupied') AND p.strRoomTypeDesc='"+strRoomTypeDesc+"') s"
		    		+ " Group by s.roomdesc"
		    		+ " ORDER BY s.roomdesc";
    				ResultSet rsRoomInfo = st.executeQuery(sql);
    			    ...
    			    while(rsRoomInfo.next())
    			    {
    			    	...
    			    	if(!rsRoomInfo.getString(10).isEmpty())
    			    	{
				    		String dateBeforeString =new clsUtilityFunctions().funGetDate("dd-MM-yyyy", rsRoomInfo.getString(6));
				    		String dateAfterString =new clsUtilityFunctions().funGetDate("dd-MM-yyyy", rsRoomInfo.getString(7));
    			    		int daysBetween=new clsUtilityFunctions().funGetDiffrenceOfDates(dateBeforeString, dateAfterString);
	    			    	//int intStayDays=Integer.parseInt(rsRoomInfo.getString(10));
	    			    	if(daysBetween >0)
	    			    	{
    			    	...
    			    	mainArrObj.put(arrObj);
    			    	
    			    }	
    		jObjStayViewData.put("RoomStatusData", mainArrObj);
		}
		catch (Exception e) 
        {
	...
	public JSONObject funGetArrivalListData(@QueryParam("POSCode") String POSCode,@QueryParam("clientCode") String clientCode,@QueryParam("PMSDate") String PMSDate)
	{
        ...
        try{
			...
			String sql="SELECT ifnull(b.strReservationNo,' '),ifnull(f.strFolioNo,' '),ifnull(e.strGuestPrefix,' '),ifnull(concat(e.strFirstName,' ',e.strMiddleName,' ',e.strLastName),' '),"
					+ " left(ifnull(DATE_FORMAT(b.dteCheckInDate,'%d-%M-%Y'),' '),6),left(ifnull(DATE_FORMAT(b.dteDepartureDate,'%d-%M-%Y'),' '),6),ifnull(sum(f.dblDebitAmt),0)"					+ " FROM tblwalkinhd a,tblcheckinhd b,tblcheckindtl c ,tblroom d,tblguestmaster e,tblfoliodtl f,tblfoliohd g "
					+ " WHERE a.strWalkinNo=b.strWalkInNo"
					+ " AND a.strClientCode='320.001' and b.strCheckInNo=c.strCheckInNo and c.strRoomNo=d.strRoomCode and a.strRoomNo=d.strRoomCode "
					...
					+ " UNION"
					+ " SELECT ifnull(a.strReservationNo,' '),ifnull(f.strFolioNo,' '),ifnull(e.strGuestPrefix,' ')"
			...
			while(resArrivalList.next())
			{
				 ...
				 arrArrivalList.put(resArrivalList.getDouble(7));
				 objArrList.put(resArrivalList.getString(2), arrArrivalList);
